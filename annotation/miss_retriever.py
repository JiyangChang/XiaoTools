...
"""
XiaoTools - Missing Retriever (MR) v0.1
Author: Jiyang Chang

Description: A simple GUI application for retrieving missing genes from GFF files.

Usage:
1. Run the application and select a GFF file generated by TransDecoder.
2. Provide a boundary position of an exon to search for in the GFF file.
3. Click the "Search" button to retrieve information about the exon and its associated isoforms.
4. The results will display the chromosome, strand, and exon positions for each matching isoform, formatted as GenomeView needed.

Note: This tool is designed for use with GFF files generated by TransDecoder and may not work correctly with other GFF formats.
"""
